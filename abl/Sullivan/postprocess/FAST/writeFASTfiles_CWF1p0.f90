...
program	writeFASTFiles
  !!!Write input files for FAST computations with ABL inflow fields. Write one FAST file at the place where CWF 1.0 is being done.
  !  use LIB_VTK_IO
...
!  write(*,*) 'Taking sizeOfReal = 4. Specified in bytes rather than word counts'
  ...
  open(unit=40,file="CWF1p0.bts",form="binary",access="sequential", RECL=2)
  write(40) id
  write(40) wtExtentZ
  write(40) wtExtentY
  write(40) 0
 ...
 do k = 1,wtExtentZ
    ...
    i = 251
    xLocCur = (i-1)*dx
    do j = 254-wtExtenty/2+1, 254+wtExtenty/2
       yLocCur = (j-1)*dy
